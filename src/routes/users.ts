...
usersRouter.get("/rank", (request, response) => {
  const { requestTimeout } = databaseConfig;
  ...
  const playersRanking = new Array();

  connection.query({
    sql: `SELECT * FROM ${oneMoveCheckmatesScoresTable}`,
    timeout: requestTimeout
  }, (error, rows) => {
    for (let row of rows) {
      let points = 0;
      ...
      for (const property in row) {
        if (property !== "username") {
          if (row[property] !== null) {
            if (10 - row[property] > 0) {
              points += 10 - row[property];
              completed++;
            }
          }
        }
    ...
    connection.query({
      sql: `SELECT * FROM ${twoMovesCheckmatesScoresTable}`,
      timeout: requestTimeout
    }, (error, rows) => {
      countPointsForPlayer(playersRanking, rows, 15);
      connection.query({
        sql: `SELECT * FROM ${threeMovesCheckmatesScoresTable}`,
        timeout: requestTimeout
      }, (error, rows) => {
        countPointsForPlayer(playersRanking, rows, 20);
        connection.query({
          sql: `SELECT * FROM ${stalematesScoresTable}`,
          timeout: requestTimeout
        }, (error, rows) => {
          countPointsForPlayer(playersRanking, rows, 10);
          connection.query({
            sql: `SELECT * FROM ${doubleAttacksScoresTable}`,
            timeout: requestTimeout
          }, (error, rows) => {
            countPointsForPlayer(playersRanking, rows, 10);
            response.send(playersRanking);
          });
        });
      });
    });
...
const countPointsForPlayer = (responseObject: any, sqlResponse: any, pointsParameter: any) => {
  for (let row of sqlResponse) {
    let completed = 0;
    let score = 0;
    for (const property in row) {
      if (property !== "username") {
        if (row[property] !== null) {
          if (pointsParameter - row[property] > 0) {
            completed++;
            score += pointsParameter - row[property];
          }
        }
      }
    }
    for (let i = 0; i < responseObject.length; i++) {
      if (responseObject.username === row.username) {
        responseObject.completed += completed;
        responseObject.points += score;
        break;
      }
    }
  }
